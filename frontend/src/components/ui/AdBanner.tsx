...
export default function AdBanner({ className = "", page }: AdBannerProps) {
  const { currentLocale } = useLanguage();
  const [banners] = useState(() => getActiveBanners(page));
  ...
  const nextBanner = useCallback(() => {
    ...
    // Отправляем аналитику завершения просмотра с точным временем
    if (viewData && currentBanner) {
      const expectedDuration = currentBanner.duration * 1000; // Точное ожидаемое время
      trackBannerViewComplete(currentBanner, expectedDuration);
    }

    setCurrentIndex((prev) => (prev + 1) % banners.length);
    setProgress(0);
  }, [hasMultipleBanners, viewData, currentBanner, banners.length, trackBannerViewComplete, currentIndex]);
  ...
  // Управление автоплеем и прогресс баром (синхронизировано)
  useEffect(() => {
    ...
    // Синхронизированный таймер для автопереключения
    intervalRef.current = setTimeout(() => {
      nextBanner();
    }, interval);

    // Синхронизированный прогресс бар
    const progressInterval = 50; // Обновляем каждые 50мс
    const increment = (progressInterval / interval) * 100;
  ...
  // Мемоизированная функция для получения пути к медиа
  const getMediaPath = useCallback((banner: AdBannerItem) => {
  ...
  // Мемоизированный компонент для рендеринга медиа
  const MediaComponent = useCallback(({ banner, isActive }: { banner: AdBannerItem; isActive: boolean }) => {
    const { src, alt } = getMediaPath(banner);
    
    const commonProps = {
      width: (isMobile ? banner.mobile : banner.desktop).width,
      height: (isMobile ? banner.mobile : banner.desktop).height,
    ...
    // Рендерим только активный баннер или если это первый баннер (для предзагрузки)
      ...
      case 'webm':
        ...
        return (
          <img
            ...
            className="w-full h-full object-cover transition-all duration-300"
            onError={(e) => {
              console.error('❌ Ошибка загрузки GIF:', src, e);
              handleImageError();
            }}
  ...
  }, [getMediaPath, currentIndex, isMobile]);
    ...
    <div className={`relative w-full h-full overflow-hidden rounded-[44px] ${className}`}>
      ...
      >
        ...
        <div className="relative w-full h-full overflow-hidden">
          {/* Рендерим только текущий баннер для оптимизации */}
          <div
            key={currentBanner.id}
            className="absolute inset-0 transition-all duration-300 opacity-100 scale-100 z-10"
            style={{
          ...
          >
            <MediaComponent banner={currentBanner} isActive={true} />
          </div>
        </div>
